...
def f_l_given_S(l, s_range, shares, masked_S, n_shares, q, sigma):
    ...
    return acc_f
def f_l_given_S_(l, s_range, shares, masked_S, n_shares, q, sigma):
    """Compute f(l|s) for all possible value of the secret s
    ...
    for i, s in enumerate(s_range):
        ...
        if s == -2:
            ...
            return prod_acc_f.sum()/(q**(n_shares -1))
            # return f_li
    #     acc_f[i] = prod_acc_f.sum()
    # return acc_f
...
def p_s_given_l_(l, s, s_range, prior_s, shares, masked_S, n_shares, q, sigma):
    ...
    """
    f_l_S = f_l_given_S(l, s_range, shares, masked_S, n_shares, q, sigma).astype(np.longdouble)
    numerator_ =  f_l_S[s]
    f_l_S = f_l_S*prior_s
    numerator =  f_l_S[s]
    denominator = f_l_S.sum()
...
def conditional_entropy_(s_range, prior_s, shares, masked_S, n_shares, q, sigma, seed=12345, op="sub", mode=1):
    ...
    return h
def precompute_p(sigma, s_range, prior_s, shares, masked_S, n_shares, q):
    maxhw = int(np.log2(q))+1
    lmin = 0 - sigma*5
    lmax = maxhw + sigma*5
    n_points = int(lmax-lmin)*15
        ...
        for i_ in range(n_points):
            for j_ in range(n_points):
                ...
                if psl == 0:
                    ...
                    gij =  fls*np.log2(psl)*prior_s[i]
                    g[i_, j_] +=gij
    with open(f"precompute_vals/{q}_{sigma}.npy", "wb") as f:
        np.save(f, int_l0)
        np.save(f, int_l1)
        np.save(f, g)
def conditional_entropy__(s_range, prior_s, shares, masked_S, n_shares, q, sigma, seed=12345, op="sub", mode=1):
    ...
    h = simpson(simpson(g, int_l1), int_l0)/q
    return h
def precompute_conditional_entropy(s_range, prior_s, shares, masked_S, n_shares, q, sigma, seed=12345, op="sub", mode=1):
    """Estimate H(S|L) with n_samples
        =1/n sum_s sum_l p(s) log2(P(s|l))
        ...
        for i_ in range(n_points):
            for j_ in range(n_points):
                ...
                if psl == 0:
                    ...
                    gij =  fls*np.log2(psl)*prior_s[i]
                    g[i_, j_] +=gij
    with open(f"precompute_vals/{q}_{sigma}_{op}.npy", "wb") as f:
        np.save(f, int_l0)
        np.save(f, int_l1)
        np.save(f, g)
    return 0
...
def MI__(s_range, prior_s, shares, masked_S, n_shares, q, sigma, seed=12345, op="sub", mode=1):
    S_size = s_range.shape[0]
...
def run_MI(q, n_seed):
    ...
    for s, m in zip(sigma, log_mie):
        rand_seed = np.random.randint(10000, 20000)
        n = int(n_seed/10**m)
        ...
        with open(f"log/log_{q}_{n_shares}_{n}.txt", "a") as f_:
            f_.write(f"{mi_sub}_{mi_add}_{n}_{s:0.4f}\n")
        mi_sub = np.log10(mi_sub)
        mi_add = np.log10(mi_add)
        I_sub[i] = mi_sub
        I_add[i] = mi_add
        i += 1
    with open(f"log/MI_{q}_{n_shares}_add_{n_seed}.npy", "wb") as f:
        np.save(f, I_add)
    with open(f"log/MI_{q}_{n_shares}_sub_{n_seed}.npy", "wb") as f:
        np.save(f, I_sub)
...
def test_f():
    ...
    mia = 0
    for i in range(5):
        r_seed = np.random.randint(10, 20)
        mi = MI__(s_range, prior_s, shares, masked_S, n_shares, q, sigma, seed=r_seed, op=op, mode=1)
        print(mi)
        mia += mi
    print("AVG", mia/5)
    # print(MI(s_range, prior_s, shares, masked_S, n_shares, q, sigma, seed=12345, op=op, mode=1))
...
def run_precompute_p(q):
    ...
    sig = sigma[-1]
    print(sig)
    # CE_low = np.zeros((15,), dtype=np.longdouble)
    # CE_high = np.zeros((15,), dtype=np.longdouble)
    shares_sub, masked_S_sub = load_precomputed_vals(q, n_shares, op="sub", mode="1")
    # shares_add, masked_S_add = load_precomputed_vals(q, n_shares, op="add", mode="1")
    print("==================OP SUB========================")
    # prec_sub = partial(precompute_p, s_range=s_range, prior_s=prior_s, shares=shares_sub, masked_S=masked_S_sub, n_shares=n_shares, q=q)
    # prec_sub(sigma_low[0])
    # precompute_p(sigma[-1], s_range, prior_s, shares_sub, masked_S_sub, n_shares, q)
    precompute_conditional_entropy(s_range, prior_s, shares_sub, masked_S_sub, n_shares, q, sigma[-1], seed=12345, op="sub", mode=1)
    return 0
if __name__ == '__main__':
    ...
    # N = tqdm([5000])
    # for n_seed in N:
    #     N.set_description(f"n_seed: {n_seed}")
    #     run_MI(q, n_seed)

    # run_precompute_p(q=3329)
