...
def f_li_given_S(li, q, sigma):
    ...
    f_li = pdf_hw[hw_set]
    return f_li
def f_li_given_S_(li, S, sigma):
    """f(li|s) for all s in S
...
def f_L_given_S(L, s_range, shares, masked_S, n_shares, q, sigma):
    """Compute f(li|s) for all possible value of the secret s multiple traces

    Parameters
    ----------
    Li: 1D array
        1 share leakage array
    ...
    -------
    f(Li|S): array size S
            f(li|S)[j] = f(li|sj): proba for all possible shares values
    f(L0|S):array size S
            f(l0|S)[j] = f(l0|sj): proba for masked state
    ...
    hw_set_u = np.unique(hw_set)
    f_L__ = {}
    f_L_ = np.ones((n_traces, shares.shape[0]))
    for i in range(n_shares-1):
        Li = np.expand_dims(L[i], axis=1)
        f_Li = np.apply_along_axis(pdf_normal, axis=1, arr=Li, mu=hw_set_u, sigma=sigma)
        idx_shares = HW(shares[:, i])
        f_L__[f"L_{i+1}"] = f_Li[:, idx_shares]
        f_L_ *= f_Li[:, idx_shares]
    f_L__["L_0"] = []
    L0 = np.expand_dims(L[n_shares-1], axis=1)
    f_L0 = np.apply_along_axis(pdf_normal, axis=1, arr=L0, mu=hw_set_u, sigma=sigma)
    f_L = []
    for masked_s in masked_S:
        idx_maskeds = HW(masked_s)
        # print("HW maskeds", idx_maskeds)
        f_L__["L_0"].append(f_L0[:, idx_maskeds])
        f_L.append(f_L_* f_L0[:, idx_maskeds])
    # pretty_print_dict(f_L__)
    # print("===============\n",f_L_)
    f_L = np.array(f_L)
    # print(f_L.shape, f_L)
    f_L____ = f_L.sum(axis=2)
    print(f_L____.shape, f_L____)
    return f_L____

def g_s_given_L(s, f_L, prior_s, n_shares, q, sigma, seed=12345, op="sub", mode=1):
    f_L_given_s = f_L[s]
    # print(f_L_given_s)
    sum_f_L_given_S = f_L.sum(axis=0)
    # print("sum", f_L.sum(axis=0), sum_f_L_given_S)
    p_s_given_L = f_L_given_s/sum_f_L_given_S
    g_s_given_L = f_L_given_s*np.log2(p_s_given_L)
    return g_s_given_L


def cond_ent_g(s_range, prior_s, n_shares, q, sigma, seed=12345, op="sub", mode=1):
    """Estimate H(S|L) by numerical integration
        H(S|L)=sum_s p(s) int_l f(l|s) log2(P(s|l))dl
    ...
    int_l0, int_l1, L = int_l(lmin=-2, lmax=14, n_points=50)
    ...
    print("F_L shape", f_L.shape)
    for i, s in enumerate(s_range):
        g = g_s_given_L(s, f_L, prior_s, n_shares, q, sigma, seed=12345, op="sub", mode=1)
        print("g shape", g.shape, int_l0.shape)
        int_g_l0 = simps(g, int_l0)
        print(int_g_l0)
        int_g = simps(int_g_l0, int_l1)
        print(int_g)
...
def test_f():
    ...
    # L0 = np.expand_dims(L[0], axis=1)
    # l00 = L0[0]
    # Zq = np.arange(q)
    # hw_set = HW(Zq)
    # hw_set_u = np.unique(hw_set)
    # fL0 = np.apply_along_axis(pdf_normal, axis=1, arr=L0, mu=hw_set_u, sigma=sigma)
    # print(fL0)
    # hw_shares = HW(shares)
    # for share in hw_shares:
    #     print(share)
